...
# Version: ios/VERSION is the marketing version (bump it to cut a release).
# Build: App Store Connect rejects a build number it has already seen, so
# this cannot stay pinned. Commit count is monotonic, needs no state file,
# and is reproducible from any checkout; CI passes its own via BUILD_NUMBER.
FA_VERSION="$(tr -d '[:space:]' < "$IOS_DIR/VERSION")"
FA_BUILD="${BUILD_NUMBER:-$(git -C "$REPO" rev-list --count HEAD 2>/dev/null || echo 1)}"
...
CONFIG="$IOS_DIR/Local.xcconfig"
{
    ...
    echo "FA_BUILD = $FA_BUILD"
    if [ -n "$TEAM" ]; then
        echo "DEVELOPMENT_TEAM = $TEAM"
        echo "CODE_SIGN_STYLE = Automatic"
    fi
} > "$CONFIG"
echo "==> Version $FA_VERSION, build $FA_BUILD"
...
MSG
    ...
    echo "==> Archiving (Release, auth: $MODE, version $FA_VERSION, build $FA_BUILD)"
